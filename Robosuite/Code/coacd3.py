...
if __name__ == "__main__":
    ...
    output_base = args.output # This will be the prefix/path for your new files
    ...
    # Using the fix from the previous interaction: pass vertices and faces directly
    ...
    # --- CHANGE THIS SECTION ---
    # Instead of exporting a single scene, export each part individually
    
    mesh_parts = []
    for i, (vs, fs) in enumerate(result):
        part_mesh = trimesh.Trimesh(vs, fs)
        # Create a unique filename for each part, e.g., 'output/hull_prefix_0.stl'
        part_filename = f"{output_base}_{i}.stl"
        part_mesh.export(part_filename)
        mesh_parts.append(part_mesh)
        print(f"Exported part {i+1}/{len(result)} to {part_filename}")

    print(f"Decomposition complete. Total parts: {len(result)}")
    # The trimesh.Scene creation and single export lines are now removed
